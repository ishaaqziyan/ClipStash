...
pub async fn submit_clip_password (
    ...
    if let Some(form)= &form.value {
        ...
        let context = ctx::PasswordRequired::new(shortcode);
        Ok(Html(renderer.render(context, &["A Password is required to view this clip"],
     )))
    }
}
...
pub mod test {
    ...
    #[test]
    fn error_on_missing_clip_id() {
        let client = client();
        let response = client.get("/clip/hdjjsdidof").dispatch();
    ...
    fn requires_password_when_applicable() {
        ...
        let response = client
            .get(format!("/clip/raw/{}", clip.shortcode.as_str()))
            .cookie(Cookie::new("password", "abcdef"))
            .dispatch();
        assert_eq!(response.status(), Status::Unauthorized);
    }
}
